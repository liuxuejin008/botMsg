...
echo -e "\n${CYAN}═══════════════════════════════════════${NC}"
echo -e "${CYAN}  BotMsg 本地测试  ($BASE)${NC}"
echo -e "${CYAN}═══════════════════════════════════════${NC}\n"
...
echo -e "\n${CYAN}[4/8] 创建机器人${NC}"
# ────────────────────────────────────────
RESP=$(curl -sS -w "\n%{http_code}" -X POST "$BASE/api/bots" \
  -H "$AUTH" -H "Content-Type: application/json" \
  -d '{"name":"TestBot"}')
CODE=$(echo "$RESP" | tail -1); BODY=$(echo "$RESP" | sed '$d')
check_status "POST /api/bots" "201" "$CODE" "$BODY"
...
# 列出机器人
RESP=$(curl -sS -w "\n%{http_code}" "$BASE/api/bots" -H "$AUTH")
CODE=$(echo "$RESP" | tail -1); BODY=$(echo "$RESP" | sed '$d')
check_status "GET /api/bots" "200" "$CODE" "$BODY"
BOT_COUNT=$(echo "$BODY" | jq '.bots | length')
...
echo -e "\n${CYAN}[5/8] 更新机器人${NC}"
# ────────────────────────────────────────
RESP=$(curl -sS -w "\n%{http_code}" -X PATCH "$BASE/api/bots/$BOT_ID" \
  -H "$AUTH" -H "Content-Type: application/json" \
  -d '{"name":"RenamedBot"}')
CODE=$(echo "$RESP" | tail -1); BODY=$(echo "$RESP" | sed '$d')
check_status "PATCH /api/bots/$BOT_ID" "200" "$CODE" "$BODY"
...
# ────────────────────────────────────────
RESP=$(curl -sS -w "\n%{http_code}" "$BASE/api/bots/$BOT_ID/messages/pull?limit=50" \
  -H "$AUTH")
CODE=$(echo "$RESP" | tail -1); BODY=$(echo "$RESP" | sed '$d')
check_status "GET /api/bots/$BOT_ID/messages/pull" "200" "$CODE" ""
...
echo -e "\n${CYAN}[8/8] 消息历史 (分页)${NC}"
# ────────────────────────────────────────
RESP=$(curl -sS -w "\n%{http_code}" "$BASE/api/bots/$BOT_ID/messages" \
  -H "$AUTH")
CODE=$(echo "$RESP" | tail -1); BODY=$(echo "$RESP" | sed '$d')
check_status "GET /api/bots/$BOT_ID/messages (历史)" "200" "$CODE" ""
