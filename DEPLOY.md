# BotMsg 部署文档 — Cloudflare Workers
...
```bash
npx wrangler d1 execute botmsg --local --command="SELECT name FROM sqlite_master WHERE type='table';"
```
...
```
✅ Successfully created DB 'botmsg'
...
```bash
npx wrangler d1 execute botmsg --remote --command="SELECT name FROM sqlite_master WHERE type='table';"
```
...
```jsonc
"vars": {
  "PUBLIC_BASE_URL": "https://botmsg.<your-subdomain>.workers.dev",
...
Uploaded botmsg
Published botmsg
  https://botmsg.<subdomain>.workers.dev
```

### 4.6 部署后验证

```bash
WORKER_URL="https://botmsg.<subdomain>.workers.dev"
...
```bash
# 生产
npx wrangler d1 execute botmsg --remote --command="SELECT COUNT(*) FROM users;"

# 本地
npx wrangler d1 execute botmsg --local --command="SELECT COUNT(*) FROM messages;"
```
...
```bash
npx wrangler d1 execute botmsg --local --file=migrations/d1/0002_add_bot_description.sql
```

3. 应用到生产：

```bash
npx wrangler d1 execute botmsg --remote --file=migrations/d1/0002_add_bot_description.sql
```
...
```bash
npx wrangler r2 object list botmsg-avatars
```
...
```bash
# 上传
npx wrangler r2 object put botmsg-avatars/test.png --file=./test.png

# 下载
npx wrangler r2 object get botmsg-avatars/test.png --file=./downloaded.png
```
...
```bash
# 列出所有 R2 key
npx wrangler r2 object list botmsg-avatars --json | jq '.[].key'

# 与数据库中的 avatar_url 对比后，手动删除孤立对象
npx wrangler r2 object delete botmsg-avatars/<orphan-key>
```
...
部署后自动获得 `https://botmsg.<subdomain>.workers.dev`，无需额外配置。

### 使用自定义域名

1. 在 Cloudflare Dashboard 中将域名添加到你的账号（需修改 DNS 指向 Cloudflare）。

2. 在 Dashboard → Workers & Pages → botmsg → Settings → Domains & Routes 添加自定义域名。

3. 或在 `wrangler.jsonc` 中添加：

```jsonc
"routes": [
  { "pattern": "botmsg.yourdomain.com/*", "zone_name": "yourdomain.com" }
]
```
...
```jsonc
"vars": {
  "PUBLIC_BASE_URL": "https://botmsg.yourdomain.com"
}
```
...
登录 https://dash.cloudflare.com → Workers & Pages → botmsg：
...
```yaml
      - name: Apply D1 migrations
        ...
        with:
          apiToken: ${{ secrets.CLOUDFLARE_API_TOKEN }}
          command: d1 execute botmsg --remote --file=migrations/d1/0001_initial.sql
```
...
```bash
npx wrangler d1 execute botmsg --remote \
...
- 确认 R2 bucket 名称为 `botmsg-avatars`
- 确认 `PUBLIC_BASE_URL` 正确（头像 URL = `PUBLIC_BASE_URL/avatars/<key>`）
- 检查 R2 中是否存在该 key：`npx wrangler r2 object list botmsg-avatars`
