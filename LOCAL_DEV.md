...
```
🌀 Executing on local database botmsg (LOCAL)
...
Your Worker has access to the following bindings:
  env.DB (botmsg)              D1 Database      local
  env.AVATARS (botmsg-avatars) R2 Bucket        local
...
```
═══════════════════════════════════════
  BotMsg 本地测试  (http://localhost:8787)
═══════════════════════════════════════
...
```bash
# 查看所有用户
npx wrangler d1 execute botmsg --local \
...
npx wrangler d1 execute botmsg --local --file=migrations/d1/0001_initial.sql
npx wrangler d1 execute botmsg --local --file=migrations/d1/0002_enhancements.sql
...
# 查看本地数据
npx wrangler d1 execute botmsg --local --command="SELECT * FROM users;"
