# BotMsg SAML 2.0 SSO 技术方案

## 1. 背景与目标
BotMsg 作为一个面向开发者和企业的数据通道(Webhook/Proxy)，随着企业用户的增加，需要支持标准的企业级身份认证协议 SAML 2.0 (Security Assertion Markup Language)，以允许企业用户通过他们现有的 Identity Provider (IdP，如 Okta, Azure AD, 飞书, 钉钉等) 直接单点登录到 BotMsg。

**主要目标**：
- 支持 SP-Initiated (Service Provider 也就是 BotMsg 发起的) 登录流程。
- 解析并验证 IdP 返回的 SAML Assertion。
- 自动为首次登录的 SSO 用户在 BotMsg 系统中分配/创建账号。
- 与现有的 JWT 认证体系打通。

## 2. 架构痛点与技术挑战
BotMsg 的核心运行在 **Cloudflare Workers** (Edge 环境) 上。
...
- **流程**：BotMsg -> 跳转到 WorkOS/Auth0 -> 跳转到企业 IdP -> 企业 IdP 返回 XML 给 WorkOS -> WorkOS 解析并转换为标准的 OIDC/JWT 返回给 BotMsg Worker。
...
sequenceDiagram
    ...
    participant App as BotMsg (前端 React/HTML)
    participant Worker as BotMsg Worker (Hono)
    ...
    Worker->>Worker: 14. 数据库(D1)自动注册/绑定用户，签发 BotMsg JWT
...
4. **Phase 4: 测试与部署**
   - 本地 `wrangler dev` 配合本地转发工具 (如 ngrok 或 BotMsg 本身) 测试 Callback。
