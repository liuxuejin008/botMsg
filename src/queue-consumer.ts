 ...
 */
import type { Env, SandboxQueueMessage, ChannelForwardRow, MessageRow } from "./types";
...
/**
 * Sandbox Queue Consumer
 *
 * 当 webhook.ts 将 SandboxQueueMessage 入队后，由本 consumer 消费：
 * 1. 从 D1 读取原始消息 + channel_forwards 列表
 * 2. 逐个 HTTP 投递到转发目标
 * 3. 全部成功 → msg.ack()；任一失败 → msg.retry()
 * 4. 超过 max_retries 后平台自动将消息转入 DLQ
 */
export async function handleSandboxQueue(
  batch: MessageBatch<SandboxQueueMessage>,
  env: Env
): Promise<void> {
        ...
        try {
          ...
          const res = await fetch(fwd.url, {
            ...
            headers: {
              ...
              "X-BotMsg-Channel-Id": String(channelId),
              "X-BotMsg-Message-Id": String(messageId),
              ...extraHeaders,
            },
 ...
 *
 * 超过 max_retries（5次）后由平台自动转入 botmsg-dlq。
 * 在此记录日志，后续可扩展为发送告警通知。
 */
export async function handleDLQ(
  ...
  for (const msg of batch.messages) {
    ...
    // 标记为最终失败
    await env.DB.prepare(
      `INSERT INTO forward_log (message_id, forward_id, attempt, error)
       VALUES (?, 0, ?, 'DLQ: max retries exceeded')`
    )
      .bind(messageId, msg.attempts + 1)
      .run();
