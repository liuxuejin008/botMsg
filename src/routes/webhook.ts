 ...
 *  c. 通过 waitUntil 异步触发转发逻辑（forwardMessage）
 */
...
async function handleWebhook(c: WebhookCtx) {
  ...
  const channel = await c.env.DB.prepare(
    "SELECT * FROM channels WHERE webhook_secret = ?"
  )
    .bind(secret)
    .first<ChannelRow>();
  ...
  if (channel.mode === "proxy") {
    ...
    // Cloudflare Workers 为无状态执行环境，无法使用内存 Promise/Channel 机制，
    // 改用 D1 轮询实现等价的"挂起→唤醒"语义。
    while (Date.now() < deadline) {
      ...
      if (pr && pr.status === "completed") {
        ...
        return new Response(pr.response_body ?? '{"ok":true}', {
          status,
          headers: respHeaders,
        });
      }
    ...
    await c.env.DB.prepare(
      "UPDATE proxy_requests SET status = 'timeout', completed_at = datetime('now') WHERE id = ? AND status = 'pending'"
    )
      .bind(requestId)
      .run();

    return c.json({ error: "gateway_timeout", message: "Client did not respond in time" }, 504);
  }
  ...
  const { forwardMessage } = await import("../lib/forwarder");
  c.executionCtx.waitUntil(forwardMessage(c.env.DB, channel.id, messageId));

  let responseBody: unknown;
  try {
    responseBody = JSON.parse(channel.sandbox_response);
  } catch {
