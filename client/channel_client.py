...
def _build_forward_headers(
    ...
    过滤掉基础设施层的 header，并追加 BotMsg 标识。
    """
    ...
    headers["X-BotMsg-Channel-Id"] = str(channel_id)
    headers["X-BotMsg-Request-Id"] = str(req_id)
...
async def _call_receiver(
    ...
    t0 = time.monotonic()
    try:
        resp = await http.request(method, url, content=body_str, headers=headers, timeout=15)
    ...
    async def _sync_receivers(self) -> None:
        ...
        try:
            ...
            if enabled:
                ...
                self._receivers = [{"url": self.api.cfg.receiver_url, "method": "POST"}]
                log.info(
